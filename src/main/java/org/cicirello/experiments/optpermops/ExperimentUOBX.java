...
public class ExperimentUOBX {
  ...
  public static int crossoverCodeToTime(
    ...
    for (int i = 0; i < SAMPLES_PER_TRIAL; i++) {
      ...
      for (int j = 0; j < p1.length(); j++) {
        useToPreventOptimizingAway += (p1.get(j) == p2.get(j) ? p1.get(j) : p2.get(j));
      }
    }
  ...
  public static void main(String[] args) {
    ...
    for (double u : rates) {
      ...
      useToPreventOptimizingAway += crossoverCodeToTime(simple, perms1, perms2);
      useToPreventOptimizingAway += crossoverCodeToTime(optimized, perms1opt, perms2opt);
    }
    System.out.println("End Warmup Phase: " + useToPreventOptimizingAway);
    System.out.println();
